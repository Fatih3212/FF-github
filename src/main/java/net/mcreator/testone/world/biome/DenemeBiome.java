...
import net.mcreator.testone.block.DenemebioyaprakBlock;
import net.mcreator.testone.block.DenemebiotoprakBlock;
import net.mcreator.testone.block.DenemebiomeyveBlock;
import net.mcreator.testone.block.DenemebiocimenBlock;
import net.mcreator.testone.block.DenemebioagacBlock;
...
public class DenemeBiome extends TestoneElements.ModElement {
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DenemebiocimenBlock.block.getDefaultState().getBlock()
							|| ground == DenemebiotoprakBlock.block.getDefaultState().getBlock())
							&& (ground2 == DenemebiocimenBlock.block.getDefaultState().getBlock()
									|| ground2 == DenemebiotoprakBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), DenemebiotoprakBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DenemebioyaprakBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DenemebioyaprakBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DenemebioagacBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												DenemebiomeyveBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DenemebioagacBlock.block.getDefaultState().getBlock()
					|| blockType == DenemebioyaprakBlock.block.getDefaultState().getBlock()
					|| blockType == DenemebiocimenBlock.block.getDefaultState().getBlock()
					|| blockType == DenemebiotoprakBlock.block.getDefaultState().getBlock();
		}
