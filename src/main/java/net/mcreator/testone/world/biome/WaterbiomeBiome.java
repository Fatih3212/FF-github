...
import net.mcreator.testone.block.WateryaprakBlock;
import net.mcreator.testone.block.WatermeyvesiBlock;
import net.mcreator.testone.block.WaterdirtBlock;
import net.mcreator.testone.block.WatercimenBlock;
import net.mcreator.testone.block.WateragacBlock;
...
public class WaterbiomeBiome extends TestoneElements.ModElement {
	@ObjectHolder("testone:waterbiome")
	public static final CustomBiome biome = null;
	public WaterbiomeBiome(TestoneElements instance) {
		super(instance, 66);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(WatercimenBlock.block.getDefaultState(),
							WaterdirtBlock.block.getDefaultState(), WaterdirtBlock.block.getDefaultState())));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == WatercimenBlock.block.getDefaultState().getBlock()
							|| ground == WaterdirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == WatercimenBlock.block.getDefaultState().getBlock()
									|| ground2 == WaterdirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), WaterdirtBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == WateryaprakBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, WateryaprakBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, WateragacBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												WatermeyvesiBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == WateragacBlock.block.getDefaultState().getBlock()
					|| blockType == WateryaprakBlock.block.getDefaultState().getBlock()
					|| blockType == WatercimenBlock.block.getDefaultState().getBlock()
					|| blockType == WaterdirtBlock.block.getDefaultState().getBlock();
		}
